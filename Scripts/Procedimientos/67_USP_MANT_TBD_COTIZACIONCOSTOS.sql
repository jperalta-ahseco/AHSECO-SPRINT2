...
BEGIN
	...
	IF (@pTipoProceso IN ('I','U','D')) BEGIN
		...
		@INDINSTACAPA = CDD.INDINSTACAPA, @INDGARANADIC = CDD.INDGARANADIC,
		...
		IF ISNULL(@INDINFOMANUAL,'') = 'S' AND ISNULL(@FLAG_MANUALES,0) < 2 BEGIN
			SET @INDCOSTEADO = 'N'
		END
		IF ISNULL(@INDINSTACAPA,'') = 'S' AND ISNULL(@FLAG_INSTACAP,0) < 2 BEGIN
			SET @INDCOSTEADO = 'N'
		END
