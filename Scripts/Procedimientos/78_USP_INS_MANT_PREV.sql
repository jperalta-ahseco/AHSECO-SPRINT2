...
/*Detalles Generales de la Solicitud*/---------------------------------------------------------------------------------------------------------------------
	BEGIN TRY
		...
		WHERE SOL.ID_SOLICITUD = @isIdSolicitud AND DESCOT.INDINSTALACION = 'S' AND ISNULL(COTDET.ELIMINADO,'N') != 'S'
------------------------------------------------------------------------------------------------------------------------------------------------------------
		--SELECT * FROM #tmpMain
...
/*Insertamos de forma parcial*/-------------------------------------------------------------------------------------------------------
		;WITH CTE_1 AS (
			...
			AND ID_COTDETALLE IN (SELECT ID FROM TBD_COTIZACIONVENTA WHERE ID_COTIZACION IN (SELECT ID_COTIZACION FROM TBM_COTIZACIONVENTA WHERE ID_SOLICITUD = @isIdSolicitud) AND ISNULL(ELIMINADO,'N') != 'S')
		),
		Numeros AS (
