...
DECLARE @SUBTOT DECIMAL(18,9)
	...
	WHERE A.ID_SOLICITUD=@CodigoSol;
		...
		SET @IGV =RTRIM(@SIMBOLO)+' '+ FORMAT(ISNULL(CAST(ISNULL(@SUBTOT * 0.18 ,0) AS MONEY),0.00) , '#,###.##') 
		SET @TOTAL = RTRIM(@SIMBOLO)+' '+ FORMAT(ISNULL(CAST(ISNULL(@SUBTOT + (@SUBTOT * 0.18) ,0) AS MONEY),0.00) , '#,###.##') 
...
SELECT CAST(B.NROITEM AS VARCHAR) NROITEM, 
	...
	RTRIM(ISNULL(C.COD_VALOR3,'')+' '+FORMAT(ISNULL((CASE WHEN B.PORCGANANCIA > 0 THEN
											ISNULL(B.VVTOTALSIGVCGAN,0.00) 
									ELSE 
											ISNULL(B.VVTOTALSIGV,0.00) 
									END)/B.CANTIDAD,0.00) , '#,###.##')) PRECIOUNITARIO,
