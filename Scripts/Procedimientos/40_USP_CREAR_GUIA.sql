...
DECLARE @SUBTOT DECIMAL(18,9)
	...
	GROUP BY NROITEM
		...
		SELECT @SUBTOTAL=RTRIM(ISNULL(@SIMBOLO,''))+' '+ CONVERT(VARCHAR, CAST(SUM(
					...
					@SUBTOT =ISNULL(SUM(
								...
								,0.00)
		FROM TBD_COTIZACIONVENTA WHERE ID_COTIZACION = @COD_COTIZACION AND INDSTOCK=(CASE WHEN @Stock='X' THEN INDSTOCK ELSE @Stock END);
	...
	INNER JOIN TBD_COTIZACIONVENTA B WITH(NOLOCK) ON A.ID_COTIZACION=B.ID_COTIZACION  AND B.INDSTOCK=(CASE WHEN @Stock='X' THEN B.INDSTOCK ELSE @Stock END)
