...
DECLARE @SUBTOT DECIMAL(18,9)
	...
	GROUP BY NROITEM
		...
		WHERE ID_COTIZACION = @COD_COTIZACION AND 
					 INDSTOCK=(CASE WHEN @Tipo = 'GP' THEN INDSTOCK ELSE @Stock END);
	...
	INNER JOIN TBD_COTIZACIONVENTA B WITH(NOLOCK) ON A.ID_COTIZACION=B.ID_COTIZACION  AND B.INDSTOCK=(CASE WHEN @Tipo = 'GP' THEN B.INDSTOCK ELSE @Stock END)
