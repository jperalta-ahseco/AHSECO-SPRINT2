...
DECLARE @SUBTOT DECIMAL(18,9)
	...
	WHERE A.ID_SOLICITUD=@CodigoSol;
		...
		SELECT @SUBTOTAL=RTRIM(ISNULL(@SIMBOLO,''))+' '+ CONVERT(VARCHAR, CAST(SUM(CASE WHEN ISNULL(MONTODSCTO,0) > 0 THEN ISNULL(VVTOTALSIGVDSCTO,0) 
									WHEN ISNULL(PORCGANANCIA,0) > 0 THEN ISNULL(VVTOTALSIGVCGAN,0) 
									ELSE ISNULL(VVTOTALSIGV,0) END)  AS MONEY),1)  ,
					@SUBTOT =ISNULL(SUM(CASE WHEN ISNULL(MONTODSCTO,0) > 0 THEN ISNULL(VVTOTALSIGVDSCTO,0) 
									WHEN ISNULL(PORCGANANCIA,0) > 0 THEN ISNULL(VVTOTALSIGVCGAN,0) 
									ELSE ISNULL(VVTOTALSIGV,0) END) ,0.00)
...
SELECT CAST(B.NROITEM AS VARCHAR) NROITEM, 
	...
	RTRIM(ISNULL(C.COD_VALOR3,''))+' '+CONVERT(VARCHAR,CAST(((CASE WHEN B.PORCGANANCIA > 0 THEN
									...
									END)/B.CANTIDAD) AS MONEY),1) PRECIOUNITARIO,
