...
/******Obtenemos la cabecera **********************************************************************************/
	SELECT 
			...
			,INSTAL.PERIODICIDAD		AS PERIODO
			,MANT.NUMFIANZA				AS NUMFIANZA
			,NUMFIANZAPP				AS NUMFIANZAPP
			,NUMFIANZAPA				AS NUMFIANZAPA
			,IIF(COTDESP.CODGARANADIC='',NULL,COTDESP.CODGARANADIC)		AS GARANTIAADIC
		INTO #tmpCabecera
		FROM [dbo].[TBM_MANT_PREV] MANT WITH(NOLOCK)
		LEFT JOIN [dbo].[TBD_DESPACHO_DIST] DESP WITH(NOLOCK) ON MANT.SERIE = DESP.NUMSERIE
		LEFT JOIN [dbo].[TBD_INSTALACION] INSTAL WITH(NOLOCK) ON DESP.NUMSERIE = INSTAL.SERIE
		...
		LEFT JOIN [dbo].[TBM_DESPACHO] DESPACHO WITH(NOLOCK) ON DESPACHO.ID_SOLICITUD = SOL.ID_SOLICITUD AND DESPACHO.ID_COTIZACION = COTIZ.ID_COTIZACION
...
/*****Obtenemos el detalle del equipo **************************************************************************/
	...
	LEFT JOIN (SELECT [NOMPRODUCTO],[DESMARCA],[CODIGOPRODUCTO],[DESC_MODELO] FROM OPENQUERY([AH-SRV4],'
		...
		WHERE B.SK_CALMA IN (''0001'',''0015'',''0017'') GROUP BY A.AR_CCODIGO,A.AR_CDESCRI,D.TG_CDESCRI,MO.TG_CDESCRI' )) RGA ON RGA.CODIGOPRODUCTO = parcial.CODITEM
...
/*Select General*/
	SELECT ID_MANT,RUC,ASESOR,RAZONSOCIAL,NUMPROCESO,TIPOPROCESO,ORDENCOMPRA FROM #tmpCabecera
