...
SET NOCOUNT ON;
	...
	DECLARE @COD_SOLICITUD BIGINT,@RUTAIMAGEN VARCHAR(200),@NUM_COTIZACION VARCHAR(9),@ENCABEZADO VARCHAR(MAX),
				   ...
				   @SIMBOLO_MONEDA VARCHAR(3),@MONTOIGV VARCHAR(25),@TOTALVENTA VARCHAR(25),@RUC_EMPRESA VARCHAR(15), @FECHA_COTIZACION DATETIME
	...
	@FECHA_COTIZACION = FEC_COTIZACION,
	...
	WHERE B.ID_COTIZACION = @COD_COTIZACION AND A.CODCOSTO='CXCD0006' AND ISNULL(B.ELIMINADO,'N') != 'S'
	AND A.CANTPREVENTIVO > 0

	--CANTIDAD DE CALIBRACIONES:
	INSERT INTO #TMP_LISTACALIB
	SELECT  A.CANTCOSTO,B.DESCRIPCION from TBD_COTIZACIONCOSTOS A
	INNER JOIN TBD_COTIZACIONVENTA B ON A.ID_COTDETALLE=B.ID
	WHERE B.ID_COTIZACION = @COD_COTIZACION AND A.CODCOSTO='CXCD0007' AND ISNULL(B.ELIMINADO,'N') != 'S'
			AND A.CANTCOSTO > 0
	...
	SELECT @RUTAIMAGEN RUTAIMAGEN,
				  ...
				  @RUC RUC,
				  CONVERT(VARCHAR,@FECHA_COTIZACION,103) FECHA,
				...
				WHERE A.ID_COTIZACION =@COD_COTIZACION AND  A.TIPOITEM IN ('PRO','ACC','SER') AND ISNULL(A.ELIMINADO,'N') != 'S'
