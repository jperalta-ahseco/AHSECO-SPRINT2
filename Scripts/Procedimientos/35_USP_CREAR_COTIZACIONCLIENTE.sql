...
SET NOCOUNT ON;
	...
	SELECT @RUTAIMAGEN RUTAIMAGEN,
				...
				SELECT CAST(A.NROITEM AS VARCHAR) NROITEM,
							...
							(RTRIM(ISNULL(@SIMBOLO_MONEDA,''))+' '+CONVERT(VARCHAR,CAST(ISNULL((CASE WHEN A.PORCGANANCIA > 0 THEN
											ISNULL(A.VVTOTALSIGVCGAN,0.00) 
									ELSE 
											ISNULL(A.VVTOTALSIGV,0.00) 
									END)/A.CANTIDAD,0.00)  AS MONEY),1) )  PRECIOUNITARIO,
				...
				WHERE A.ID_COTIZACION =@COD_COTIZACION AND  A.TIPOITEM IN ('PRO','ART','SER')
				ORDER BY A.NROITEM;
