...
SET NOCOUNT ON
	...
	BEGIN
		...
		IF(@IsID_CONTACTO = 0)
		BEGIN
			IF EXISTS (SELECT 1 FROM [TBM_INSTAL_CONTACTO] WHERE NUM_DOC = @IsNUM_DOC AND TIPO_DOC = @IsTIPO_DOC AND NUMREQ = @IsNUMREQ)
			BEGIN
				SET @COD = -1;
	...
	IF (@IsTipoProceso = 'U')
	BEGIN
		IF EXISTS (SELECT 1 FROM [TBM_INSTAL_CONTACTO] WHERE NUM_DOC = @IsNUM_DOC AND TIPO_DOC = @IsTIPO_DOC AND ID_ASIG != @IsID_ASIG)
			BEGIN
				SET @COD = -1;
				SET @MSG = 'Ya existe un empleado con el mismo documento.'
			END
