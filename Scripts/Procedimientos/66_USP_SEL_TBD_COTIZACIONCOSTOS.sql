...
BEGIN
	...
	,[CODCICLOPREVENT]
	,[CODUBIGEODEST]
	,ISNULL(UBI.NOMDEPARTAMENTO,'')+'/'+ISNULL(UBI.NOMPROVINCIA,'')+'/'+ISNULL(UBI.NOMDISTRITO,'') UBIGEODEST 
	,[DIRECCION]
	,[AMBIENTEDEST]
	,[NROPISO]
	,[MONTOUNICOSTO]
	...
	LEFT JOIN [dbo].[TBM_UBIGEO] UBI ON UBI.CODUBIGEO = CC.CODUBIGEODEST
	WHERE
	(ISNULL(@pId,0) = 0 OR CC.ID = @pId)
	AND (ISNULL(@pId_Cotizacion,0) = 0 OR ID_COTDETALLE IN (SELECT T1.ID FROM TBD_COTIZACIONVENTA T1 WHERE T1.ID_COTIZACION = @pId_Cotizacion AND ELIMINADO != 'S'))
