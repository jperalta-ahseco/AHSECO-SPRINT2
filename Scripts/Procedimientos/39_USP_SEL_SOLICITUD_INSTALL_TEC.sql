USE [DB_AHSECO]
GO

CREATE OR ALTER PROCEDURE [dbo].[USP_SEL_SOLICITUD_INSTALL_TEC]
(
/*=======================================================================================================
	Nombre:				Fecha:			Descripcion:
	Diego Bazalar		24.10.24		Realiza el select del detalle de la solicitud de venta y de la cotización para la bandeja de instalación técnica.
	[USP_SEL_SOLICITUD_INSTALL_TEC] 2
  =======================================================================================================*/
	@isIdSolicitud BIGINT
)
AS
BEGIN

SET NOCOUNT ON;
	
	SELECT
		SOL.ID_SOLICITUD					AS ID_SOLICITUD
		,ID_WORKFLOW						AS ID_WORKFLOW
		,datos.VALOR1						AS EMPRESA
		,SOL.COD_EMPRESA					AS CODEMPRESA
		,IDCLIENTE							AS IDCLIENTE
		,RUC								AS RUC
		,RAZONSOCIAL						AS RAZONSOCIAL
		,CONCAT(ISNULL(ubi.[NOMDEPARTAMENTO],''),' / ',ISNULL(ubi.[NOMPROVINCIA],''),' / ',ISNULL(ubi.[NOMDISTRITO],'')) AS UBIGEO
		,ASESORVENTA						AS ASESORVENTA
		,COTI.NOMBRECONTACTO				AS NOMBRECONTACTO
		,COTI.TELEFONOCONTACTO				AS TELEFONOCONTACTO
		,COTI.GARANTIA						AS GARANTIA
		,ISNULL(CONTACT.ESTABLECIMIENTO,'')	AS ESTABLECIMIENTO
		,ISNULL(CONTACT.CARGOCONTACTO,'')	AS CARGOCONTACTO
		,EMAILCONTACTO						AS EMAILCONTACTO
		,SOL.TIPOVENTA						AS TIPOVENTA
		,TIPO.VALOR1						AS TIPO
		,CONVERT(VARCHAR(10),FECHA_SOL,103)	AS FECHA_SOL
		,SOL.ESTADO							AS ESTADO
		,TIPOPROCESO						AS TIPOPROCESO
		,NROPROCESO							AS NROPROCESO
		,DESP.NUMORDEN						AS NUMORDEN
		,ISNULL(CONVERT(VARCHAR(10),DESP.FECHAMAX,103),'') AS FECHAMAX
	INTO #tmpSolicitudVenta										---Insertamos datos de la solicitud de venta
	FROM [dbo].[TBM_SOLICITUDVENTA] AS SOL WITH(NOLOCK)
	LEFT JOIN [dbo].[TBM_CLIENTES] AS cli WITH(NOLOCK) ON cli.ID = SOL.IDCLIENTE
	LEFT JOIN [dbo].[TBM_COTIZACIONVENTA] AS COTI WITH(NOLOCK) ON SOL.ID_SOLICITUD = COTI.ID_SOLICITUD AND COTI.ESTADO = 'A'
	LEFT JOIN [dbo].[TBM_CONTACTOS] AS CONTACT WITH(NOLOCK) ON COTI.IDCONTACTO = CONTACT.IDCONTACTO
	LEFT JOIN [dbo].[TBM_UBIGEO] AS ubi WITH(NOLOCK) ON ubi.CODUBIGEO = cli.CODUBIGEO
	LEFT JOIN [dbo].[TBD_DATOS_GENERALES] datos WITH(NOLOCK) ON datos.PARAMETRO = SOL.COD_EMPRESA
	LEFT JOIN [dbo].[TBD_DATOS_GENERALES] AS TIPO ON TIPO.COD_VALOR1 = SOL.TIPO_SOL 
	LEFT JOIN (SELECT COD_VALOR1,VALOR1 FROM [dbo].[TBD_DATOS_GENERALES] WHERE DOMINIO = 'FLOWSOL') AS FLUJO ON FLUJO.COD_VALOR1 = CAST(SOL.ID_FLUJO AS NVARCHAR)
	LEFT JOIN (SELECT ID_SOLICITUD,FECHAMAX,NUMORDEN FROM TBM_DESPACHO WITH(NOLOCK) GROUP BY ID_SOLICITUD,FECHAMAX,NUMORDEN) DESP ON SOL.ID_SOLICITUD=DESP.ID_SOLICITUD
	WHERE SOL.ID_SOLICITUD = @isIdSolicitud



	SELECT
		DETALLE.ID
		,RGA.CODIGOPRODUCTO
		,DESCRIPCION
		,RGA.DESMARCA
		,ISNULL(RGA.[DESC_MODELO],'')	AS MODELO
		,CANTIDAD
		,COTIZ.GARANTIA
		,ISNULL(DESPACHO.INDFIANZA,'N') INDFIANZA
		,DESPACHO.NUMFIANZA
		,ISNULL(DESPACHO.DIMENSIONES,'') DIMENSIONES
		,ISNULL(DESPACHO.MONTOPPRINC,0) MONTOPPRINC
		,ISNULL(DESPACHO.MONTOPACCE	,0) MONTOPACCE
		,NUM.NUM_INST
		,NUM.NUM_PROG
	INTO #tmpDetalleCotiz
	FROM [dbo].[TBD_COTIZACIONVENTA] AS DETALLE WITH(NOLOCK)
	LEFT JOIN [dbo].[TBM_COTDET_DESPACHO] DESPACHO WITH(NOLOCK) ON DESPACHO.ID_COTDETALLE = DETALLE.ID
	LEFT JOIN [dbo].[TBM_COTIZACIONVENTA] AS COTIZ WITH(NOLOCK) ON DETALLE.ID_COTIZACION = COTIZ.ID_COTIZACION AND ID_SOLICITUD = @isIdSolicitud AND COTIZ.ESTADO = 'A'
	LEFT JOIN (SELECT A.ID AS ID_COTDETALLE,  
							COUNT(B.FECHAINSTALACION) NUM_INST,
							COUNT(C.FECHAPROGRAMACION) NUM_PROG
							from TBD_COTIZACIONVENTA A WITH(NOLOCK)
							LEFT JOIN TBD_DESPACHO_DIST B WITH(NOLOCK) ON A.ID=B.ID_COTDETALLE AND B.FECHAINSTALACION IS NOT NULL
							LEFT JOIN TBD_DESPACHO_DIST C WITH(NOLOCK) ON A.ID=C.ID_COTDETALLE AND C.FECHAPROGRAMACION IS NOT NULL
							WHERE ISNULL(ELIMINADO,'N') != 'S'
							GROUP BY A.ID
	) NUM ON DETALLE.ID = NUM.ID_COTDETALLE
	LEFT JOIN (SELECT [NOMPRODUCTO],[DESMARCA],[CODIGOPRODUCTO],[DESC_MODELO] FROM OPENQUERY([AH-SRV4],'
	SELECT
		A.AR_CCODIGO CODIGOPRODUCTO,
		A.AR_CDESCRI NOMPRODUCTO,
		D.TG_CDESCRI DESMARCA,
		MO.TG_CDESCRI AS DESC_MODELO
	FROM  [RSFACCAR].[dbo].[AL0007ARTI] A WITH(NOLOCK)
	LEFT JOIN  [RSFACCAR].[dbo].[AL0007STOC] B WITH(NOLOCK) ON A.AR_CCODIGO=B.SK_CCODIGO
	LEFT JOIN  [RSFACCAR].[dbo].[AL0007TABL] D WITH(NOLOCK) ON D.TG_CCOD=''V7'' AND A.AR_CMARCA=D.TG_CCLAVE 
	LEFT JOIN  [RSFACCAR].[dbo].[AL0007TABL] MO WITH(NOLOCK) ON MO.TG_CCOD = ''39'' AND A.AR_CMODELO = MO.TG_CCLAVE
	LEFT JOIN  [RSFACCAR].[dbo].[AL0007SERI] F WITH(NOLOCK) ON A.AR_CCODIGO=F.SR_CCODIGO AND B.SK_CALMA=F.SR_CALMA AND F.SR_NSKDIS<>0 
	WHERE B.SK_CALMA IN (''0001'',''0015'',''0017'') GROUP BY A.AR_CCODIGO,A.AR_CDESCRI,D.TG_CDESCRI,MO.TG_CDESCRI' )) RGA ON RGA.CODIGOPRODUCTO = DETALLE.CODITEM
	WHERE ID_SOLICITUD = @isIdSolicitud  AND DETALLE.TIPOITEM = 'PRO' AND DESPACHO.INDINSTALACION = 'S' AND ISNULL(ELIMINADO,'N') != 'S'

	SELECT	
			ROW_NUMBER() OVER(ORDER BY DESPACHO.ID ) CONTADOR
			,COTDET.ID AS ID_DETALLE
			,DESPACHO.ID AS ID_DESPACHO
			,COTDET.CODIGOPRODUCTO		
			,COTDET.DESCRIPCION		
			,COTDET.DESMARCA
			,ISNULL(COTDET.MODELO,'') MODELO
			,ISNULL(DESPACHO.NUMSERIE,'') NUMSERIE
			--,COTCOST.NUMSEC
			,COTDET.GARANTIA
			--,CANTPREVENTIVO
			--,CODCICLOPREVENT
			--,SUBSTRING(CODUBIGEODEST, 0,3) CODDEPARTAMENTO 
			--,CONCAT(UBI.NOMDEPARTAMENTO, ' / ', UBI.NOMPROVINCIA, ' / ', UBI.NOMDISTRITO) AS DESCUBIGEODEST
			--,CODUBIGEODEST CODUBIGEODEST
			--,DIRECCION
			--,ISNULL(NROPISO,1) NROPISO
	INTO #tmpDespachoParcial
	FROM [dbo].#tmpDetalleCotiz AS COTDET
	--LEFT JOIN [dbo].[TBD_COTIZACIONCOSTOS] AS COTCOST WITH(NOLOCK) ON COTCOST.ID_COTDETALLE  = COTDET.ID AND COTCOST.CODCOSTO = 'CXCD0002'
	LEFT JOIN [TBD_DESPACHO_DIST] DESPACHO WITH(NOLOCK) ON COTDET.ID  =  DESPACHO.ID_COTDETALLE
	--LEFT JOIN [dbo].[TBM_UBIGEO] AS UBI WITH(NOLOCK) ON UBI.CODUBIGEO = COTCOST.CODUBIGEODEST

	/*Insertamos de forma parcial*/-------------------------------------------------------------------------------------------------------
	;WITH CTE_1 AS (
		SELECT 
			ID_COTDETALLE,
			CANTCOSTO,
			CODUBIGEODEST,
			DIRECCION,
			NROPISO
		FROM TBD_COTIZACIONCOSTOS 
		WHERE CODCOSTO = 'CXCD0002' 
		AND ID_COTDETALLE IN (SELECT ID FROM TBD_COTIZACIONVENTA WHERE ID_COTIZACION IN (SELECT ID_COTIZACION FROM TBM_COTIZACIONVENTA WHERE ID_SOLICITUD = @isIdSolicitud))
	),
	Numeros AS (
		SELECT ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS n
		FROM master.dbo.spt_values
	)
	SELECT
	ROW_NUMBER () OVER( ORDER BY ID_COTDETALLE) CUENTA,
	*
	INTO #cantidadCostos
	FROM CTE_1 t
	CROSS JOIN Numeros n
	WHERE n.n <=t.CANTCOSTO

/*Insertamos de forma parcial preventivos*/-------------------------------------------------------------------------------------------------------
	;WITH CTE_1 AS (
		SELECT
			ID_COTDETALLE,
			CANTCOSTO,
			CANTPREVENTIVO,
			CODCICLOPREVENT,
			CODUBIGEODEST
		FROM TBD_COTIZACIONCOSTOS
		WHERE CODCOSTO = 'CXCD0006'
		AND ID_COTDETALLE IN (SELECT ID FROM TBD_COTIZACIONVENTA WHERE ID_COTIZACION IN (SELECT ID_COTIZACION FROM TBM_COTIZACIONVENTA WHERE ID_SOLICITUD = @isIdSolicitud))
	),
	Numeros AS (
		SELECT ROW_NUMBER() OVER (ORDER BY (SELECT NULL)) AS n
		FROM master.dbo.spt_values
	)
	SELECT
	ROW_NUMBER () OVER( ORDER BY ID_COTDETALLE) CUENTA,
	*
	INTO #cantidadPreventivos
	FROM CTE_1 t
	CROSS JOIN Numeros n
	WHERE n.n <=t.CANTCOSTO


	SELECT 
			ROW_NUMBER () OVER( ORDER BY CONT.ID_COTDETALLE) CUENTA
			--,'' NUMSEC
			,PREV.CANTPREVENTIVO
			,CODCICLOPREVENT
			,SUBSTRING(CONT.CODUBIGEODEST, 0,3) CODDEPARTAMENTO 
			,CONCAT(UBI.NOMDEPARTAMENTO, ' / ', UBI.NOMPROVINCIA, ' / ', UBI.NOMDISTRITO) AS DESCUBIGEODEST
			,CONT.CODUBIGEODEST CODUBIGEODEST
			,CONT.DIRECCION
			,ISNULL(CONT.NROPISO,1) NROPISO
	INTO #CostosFinal
	FROM #cantidadCostos CONT
	LEFT JOIN #cantidadPreventivos PREV ON CONT.CUENTA = PREV.CUENTA
	LEFT JOIN [dbo].[TBM_UBIGEO] AS UBI WITH(NOLOCK) ON UBI.CODUBIGEO = CONT.CODUBIGEODEST

	SELECT 
		* 
	INTO #tmpDespacho
	FROM #tmpDespachoParcial desp
	LEFT JOIN #CostosFinal costos ON costos.CUENTA = desp.CONTADOR

	SELECT * FROM #tmpSolicitudVenta --Detalle de Solicitud
	SELECT * FROM #tmpDetalleCotiz	 --Detalle de Cotización
	SELECT * FROM #tmpDespacho--Detalle de Costos y despacho

	SET NOCOUNT OFF;
END
