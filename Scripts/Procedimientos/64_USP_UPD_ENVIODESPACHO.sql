...
DECLARE @CODIGO BIGINT,@MSG VARCHAR(250)
	...
	BEGIN TRY
		...
		SET ENVIOGP=CASE WHEN @ENVIOGP> 1 THEN ENVIOGP ELSE @ENVIOGP END,
		   ...
		   ENVIOFC = CASE WHEN @ENVIOFC > 1 THEN ENVIOFC ELSE @ENVIOFC END,
		   ESTAPROB = CASE WHEN @ENVIOBO> 1 THEN ESTAPROB ELSE '' END,
		   OBSERVACION = CASE WHEN @ENVIOBO> 1 THEN OBSERVACION ELSE '' END,
		   USR_MOD = @USER,
		   FEC_MOD = GETDATE()
