...
BEGIN
	...
	FOR
	SELECT ID FROM TBD_COTIZACIONVENTA WHERE ID_COTIZACION = @isIdCotizacion AND ISNULL(ELIMINADO,'N') != 'S';
	SET NOCOUNT ON;
	...
	BEGIN
		...
		DECLARE @CANT_COSTOS INT,@CANT_VAL INT,@INDVALORIZADO VARCHAR(1),@CANTCOSTOS INT, @ESTADOSOL VARCHAR(5) 

		SELECT @ESTADOSOL = ESTADO FROM TBM_SOLICITUDVENTA WITH(NOLOCK) WHERE ID_SOLICITUD = @isID_SOLICITUD

		IF @ESTADOSOL = 'CVAL'
		BEGIN
			SELECT @CANT_COSTOS=COUNT(1) FROM TBD_COTIZACIONVENTA WHERE ELIMINADO='N' AND ID_COTIZACION=@isIdCotizacion
			SELECT @CANT_VAL=COUNT(1) FROM TBD_COTIZACIONVENTA WHERE ELIMINADO='N' AND ID_COTIZACION=@isIdCotizacion AND  ISNULL(VVENTAUNI,0) != 0
			SELECT @CANTCOSTOS= COUNT(1) FROM TBD_COTIZACIONCOSTOS 
								WHERE ID_COTDETALLE IN(SELECT ID FROM TBD_COTIZACIONVENTA WHERE ID_COTIZACION=@isIdCotizacion) AND ELIMINADO=0
		...
		--IF @isPORCDSCTO IS NULL
		--BEGIN
		--UPDATE TBM_COTIZACIONVENTA SET INDDSCTOREQAPROB = NULL, INDDSCTOAPROB = NULL WHERE ID_COTIZACION = @isIdCotizacion
		--	UPDATE TBD_COTIZACIONVENTA SET MONTODSCTO = NULL,VVTOTALSIGVDSCTO = NULL WHERE ID_COTIZACION = @isIdCotizacion;
		--END

		--IF ISNULL(@isPORCDSCTO_ACTUAL,0) <> ISNULL(@isPORCDSCTO,0) 
		--BEGIN
		--	UPDATE TBM_COTIZACIONVENTA SET INDDSCTOREQAPROB = NULL, INDDSCTOAPROB = NULL WHERE ID_COTIZACION = @isIdCotizacion
		--	UPDATE TBD_COTIZACIONVENTA SET MONTODSCTO = NULL,VVTOTALSIGVDSCTO = NULL WHERE ID_COTIZACION = @isIdCotizacion;
		--END
		...
		SET @MSG ='Registro Insertado con éxito'
	END
	IF (@IsTipoProceso = 'D' ) --Habilita Descuento o Inhabilita descuento. 
	BEGIN
		UPDATE TBM_COTIZACIONVENTA
		SET PORCDSCTO			= @isPORCDSCTO
			,INDDSCTOREQAPROB	= @isINDDSCTOREQAPROB
			,INDDSCTOAPROB		= @isINDDSCTOAPROB
			,USR_MOD			= @isUsrEjecuta
			,FEC_MOD			= @isFecEjecucion
