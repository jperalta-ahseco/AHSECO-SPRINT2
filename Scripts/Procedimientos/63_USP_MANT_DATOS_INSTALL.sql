...
AS
BEGIN
	DECLARE @CODASIG BIGINT, @MSG VARCHAR(100), @NUMSERIE VARCHAR(100)
	SET NOCOUNT ON;

	SELECT @NUMSERIE = NUMSERIE FROM [dbo].[TBD_DESPACHO_DIST] WHERE ID = @IsID_DESPACHO

	UPDATE [dbo].[TBD_INSTALACION] SET SERIE = @NUMSERIE WHERE ID_DESPACHO_DIST = @IsID_DESPACHO
