...
BEGIN
	...
	SELECT TOP 1 @IDCOTIZACION = ID_COTIZACION, @VVENTAUNI = VVENTAUNI, @PORCGANANCIA = PORCGANANCIA
	FROM TBD_COTIZACIONVENTA WHERE ID = @pIDCOTDETALLE;
	
	SELECT TOP 1 @PORCDSCTO = PORCDSCTO, @INDDSCTOREQAPROB = INDDSCTOREQAPROB, @INDDSCTOAPROB = INDDSCTOAPROB FROM TBM_COTIZACIONVENTA WHERE ID_COTIZACION = @IDCOTIZACION;
	...
	IF ISNULL(@PORCDSCTO,0) > 0 AND ISNULL(@VVENTAUNI,0) > 0 AND 
	(( ISNULL(@INDDSCTOREQAPROB,'') = 'S' AND ISNULL(@INDDSCTOAPROB,'') = 'S' ) OR ( ISNULL(@INDDSCTOREQAPROB,'') <> 'S' )) BEGIN
