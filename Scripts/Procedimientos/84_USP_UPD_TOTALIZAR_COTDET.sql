...
BEGIN
	...
	IF ISNULL(@TIPOITEM,'') = 'SER' BEGIN
		...
		IF ISNULL(@VVENTAUNI,0) > 0 BEGIN
			UPDATE TBD_COTIZACIONVENTA SET VVTOTALSIGV = ISNULL(CANTIDAD,0) * ISNULL(VVENTAUNI,0) WHERE ID = @pIDCOTDETALLE
		END
		ELSE BEGIN
	...
	IF ISNULL(@TIPOITEM,'') = 'PRO' BEGIN
		...
		IF ISNULL(@VVENTAUNI,0) > 0 BEGIN
			UPDATE TBD_COTIZACIONVENTA SET VVTOTALSIGV = @VALORVENTACOTDET + ISNULL(@TOTALCOSTO,0) WHERE ID = @pIDCOTDETALLE
		END
		ELSE BEGIN
			UPDATE TBD_COTIZACIONVENTA SET VVTOTALSIGV = NULL WHERE ID = @pIDCOTDETALLE
		END
		...
		IF ISNULL(@PORCDSCTO,0) > 0 AND ISNULL(@VVENTAUNI,0) > 0 BEGIN
			...
			WHERE TIPOITEM IN ('PRO') AND ID_COTIZACION = @IDCOTIZACION
			...
			IF (SELECT VVTOTALSIGVDSCTO FROM TBD_COTIZACIONVENTA WHERE TIPOITEM IN ('PRO') AND ID_COTIZACION = @IDCOTIZACION) < ISNULL(@VVENTAUNI,0)
			BEGIN
				UPDATE TBM_COTIZACIONVENTA SET INDDSCTOREQAPROB = 'S' 
				WHERE ID_COTIZACION IN (SELECT T1.ID_COTIZACION FROM TBD_COTIZACIONVENTA T1 WHERE T1.TIPOITEM IN ('PRO') AND T1.ID_COTIZACION = @IDCOTIZACION
		...
		IF ISNULL(@INDDSCTOREQAPROB,' ') = 'S' AND ISNULL(@INDDSCTOAPROB,' ') = 'N' BEGIN
