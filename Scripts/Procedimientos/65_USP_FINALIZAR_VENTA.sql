...
BEGIN
	...
	SELECT * FROM TBM_DOCUMENTO WHERE ID_WORKFLOW=20759 AND COD_TIPODOC='DVT03' 

	SELECT * FROM TBD_DATOS_GENERALES WHERE DOMINIO='TIPODOC' AND COD_VALOR1=1
	...
	IF EXISTS(SELECT 1 FROM TBM_COTDET_DESPACHO A
						...
						WHERE A.INDINSTALACION='S' AND C.ID_SOLICITUD=@isIdSolicitud)
	BEGIN
	...
	BEGIN
			...
			IF(@TIPOSOL='TSOL04')
			BEGIN
					IF EXISTS(SELECT 1 FROM TBM_DESPACHO WHERE ISNULL(NUMGUIAREM,'')='')
					BEGIN
