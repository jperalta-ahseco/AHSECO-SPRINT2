...
BEGIN
	...
	CREATE TABLE #TMP_LISTAUNIDAD(ID INT IDENTITY(1,1),ID_COSTO BIGINT,ID_COTDETALLE BIGINT,CANTIDAD INT,STOCK VARCHAR(1));
	...
	DECLARE @ID_COSTO BIGINT,@ID_COTDETALLE BIGINT,@CANTIDAD INT
	...
	SELECT @TIPO_SOL=TIPO_SOL FROM TBM_SOLICITUDVENTA WHERE ID_SOLICITUD =@CODSOLICITUD;
	 ...
	 --REGISTRO DE DESPACHO
		...
		SELECT  CASE WHEN STOCK > 0 THEN 'S'  ELSE 'N' END STOCK FROM STOCKS;
		...
		BEGIN
			...
			IF(@TIPO_SOL IN ('TSOL05','TSOL02','TSOL03')) --SE GENERA LAS SERIES PARA TIPO: VENTA EQUIPO, RESPUESTOS Y RESPUESTO Y SERVICIO
			BEGIN
		
				INSERT INTO #TMP_LISTAUNIDAD
				SELECT ID,ID_COTDETALLE,CANTDESPACHO,STOCK
				FROM (
						SELECT A.ID,A.ID_COTDETALLE,A.CANTDESPACHO,
						CASE WHEN B.STOCK > 0 THEN 'S' ELSE 'N' END AS STOCK FROM TBD_COTIZACIONCOSTOS A WITH(NOLOCK)
						INNER JOIN TBD_COTIZACIONVENTA B WITH(NOLOCK) ON A.ID_COTDETALLE=B.ID
						WHERE A.ID_COTDETALLE IN (
						SELECT ID FROM TBD_COTIZACIONVENTA WHERE  ID_COTIZACION=8)
				) X
				WHERE X.STOCK = @COD_STOCK
				...
				BEGIN
					...
					SELECT @ID_COSTO=ID_COSTO,
								 @ID_COTDETALLE=ID_COTDETALLE,
								 @CANTIDAD=CANTIDAD,
								 @K = 1
					...
					WHERE ID= @J
						...
						WHILE(@K <=@CANTIDAD)
						BEGIN
							INSERT INTO TBD_DESPACHO_DIST(ID_DESPACHO,ID_COTCOSTOS,NUMSEC,USR_REG,FEC_REG)
							VALUES(@ID_DESPACHO,@ID_COSTO,@K ,@USRREG,GETDATE())
