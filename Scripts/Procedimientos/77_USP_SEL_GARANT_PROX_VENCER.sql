...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	FROM [dbo].[TBD_DESPACHO_DIST] AS DESPACHO WITH(NOLOCK)
 	LEFT JOIN (SELECT ID, ID_COTIZACION FROM [dbo].[TBD_COTIZACIONVENTA] WITH(NOLOCK) WHERE ISNULL(ELIMINADO,'N') != 'S') AS COTDET ON COTDET.ID = DESPACHO.ID_COTDETALLE 
	...
	LEFT JOIN [dbo].[TBD_DATOS_GENERALES] AS GARAN WITH(NOLOCK) ON GARAN.DOMINIO = 'GARANTIAS' AND GARAN.PARAMETRO = COTDET_DESP.CODGARANADIC
...
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	SELECT
		...
		,FECHAINSTALACION
		,VALORGARANTIA
		,ISNULL(NMESGARANADIC,0)			AS MESESADICIONAL
		,FECHAVENCIMIENTO
	FROM #tmpDetalle AS DETALLE
	LEFT JOIN [dbo].[TBD_COTIZACIONVENTA] AS COTDET WITH(NOLOCK) ON COTDET.ID = DETALLE.ID_COTDETALLE
	...
	WHERE FECHAVENCIMIENTO BETWEEN @FecHoy AND @FecFinal AND ISNULL(ELIMINADO,'N') != 'S'
