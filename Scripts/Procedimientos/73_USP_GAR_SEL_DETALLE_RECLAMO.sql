...
/****Obtenemos el número de preventivos *********************************************************************/
	...
	WHERE RECLAMO.ID_RECLAMO = @IsReclamo AND ISNULL(COTDET.ELIMINADO,'N') != 'S'
	...
	SELECT
			...
			,CONVERT(VARCHAR(10),DATEADD(MONTH,CAST(ISNULL(NMESGARANADIC,0) AS INT),FECHAVENCIMIENTO),103) AS FECHAVENCIMIENTO
		...
		LEFT JOIN [dbo].[TBM_COTIZACIONVENTA] coti WITH(NOLOCK) ON coti.ID_SOLICITUD = RECLAMO.ID_SOLICITUD AND coti.ESTADO = 'A'
		LEFT JOIN [dbo].[TBM_DESPACHO] despacho WITH(NOLOCK) ON despacho.ID_SOLICITUD = RECLAMO.ID_SOLICITUD AND despacho.ID_COTIZACION = coti.ID_COTIZACION
