...
BEGIN
	...
	SET @SQL = '
	SELECT '+ISNULL(@TOP,'')+'
	RTRIM(A.AR_CCODIGO) AS COD_ARTICULO,
	A.AR_CDESCRI AS DESC_ARTICULO,
	RTRIM(UM.TG_CCLAVE) AS COD_UNIDAD,
	UM.TG_CDESCRI AS DESC_UNIDAD,
	RTRIM(FA.TG_CCLAVE) AS COD_FAMILIA,
	FA.TG_CDESCRI AS DESC_FAMILIA,
	RTRIM(LI.LI_CCODLIN) AS COD_LINEA,
	LI.LI_CDESLIN AS DESC_LINEA,
	RTRIM(MA.TG_CCLAVE) AS COD_MARCA,
	MA.TG_CDESCRI AS DESC_MARCA,
	ST.SK_NSKDIS AS STOCK_DISPO,
	A.AR_NPRECI4 AS PRECIO_REF,
	A.AR_CMONCOM AS COD_MONCOM,
	CASE A.AR_CMONCOM WHEN ''MN'' THEN ''SOLES'' WHEN ''US'' THEN ''DOLARES'' END AS DESC_MONCOM,
	RTRIM(A.AR_CMODELO) AS COD_MODELO,
	...
	IF ISNULL(@pAR_CDESCRI,'') <> '' BEGIN
		SET @SQL = @SQL + ' AND REPLACE(A.AR_CDESCRI,'' '','''') = REPLACE('''+TRIM(@pAR_CDESCRI)+''','' '','''') ';
	END;
