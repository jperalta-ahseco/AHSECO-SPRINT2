...
namespace AHSECO.CCL.FRONTEND.Controllers.Ventas
{
    public class BandejaSolicitudesVentasController : Controller
    {
        ...
        public ActionResult SolicitudVenta()
        {
            ...
            if (numSol != null)
            {
                                ...
                                soli.Tipo_Sol == ConstantesDTO.SolicitudVenta.TipoSolicitud.ServiciosyRepuestos)
                            {
                                if (validarDespacho.Result.EnvioServicio == 0)
                                {
                                    if (soli.Tipo_Sol == ConstantesDTO.SolicitudVenta.TipoSolicitud.ServiciosyRepuestos)
                                    {
                                        ...
                                        {
                                            if (validarDespacho.Result.EnvioBOSinStock == 0 && validarDespacho.Result.ContadorSinStock > 0)
                                            {
                                                ViewBag.Btn_GuiaBOTotal = "";
                                                if (validarDespacho.Result.GenerarGuiaBOSinStock > 0)
                                                {
                                            ...
                                            else if (validarDespacho.Result.EnvioBOSinStock == 1 && validarDespacho.Result.ContadorSinStock > 0
                                                && validarSinStock.Result.EstadoAprobacion == "IMP" && validarDespacho.Result.EnvioGPSinStock == 0)
                                            {
                                                ViewBag.Btn_GuiaPedidoTotal = "";
                                        ...
                                        if (validarDespacho.Result.ContadorSinStock > 0 && validarDespacho.Result.ContadorConStock > 0)
                                        {
                                            if (validarDespacho.Result.EnvioBOSinStock == 0 && validarDespacho.Result.ContadorSinStock > 0)
                                            {
                                                ...
                                                && validarSinStock.Result.EstadoAprobacion == "IMP" && (validarDespacho.Result.EnvioGPSinStock == 0 || validarDespacho.Result.EnvioGPConStock == 0))
                                            {
                                                ViewBag.Btn_GuiaPedidoTotal = "";
                                                if (validarDespacho.Result.GenerarGuiaPedidoConStock > 0 ||
                                                    validarDespacho.Result.GenerarGuiaPedidoSinStock > 0)
                                        ...
                                        if (validarDespacho.Result.ContadorConStock > 0 && validarDespacho.Result.ContadorSinStock > 0)
                                        {
                                            if (validarDespacho.Result.GestionLogConStock > 0 && validarDespacho.Result.GestionLogSinStock > 0)
                                            {
                                                ...
                                                if (soli.Tipo_Sol == ConstantesDTO.SolicitudVenta.TipoSolicitud.Servicio && validarDespacho.Result.GenerarGuiaManuscrita > 0)
                                                {
                                    ...
                                    {
                                        ViewBag.Btn_GuiaManuscritaTotal = "inline-block";
                                        if (validarDespacho.Result.GenerarGuiaManuscrita > 0)
                                        {
                                            ViewBag.Btn_EnviarServicio = "inline-block";
                                        }
                ...
                if (rptaCotizacion.Result.Any())
                {
                    ...
                    var smth = ViewBag.IdCotizacion;
                    VariableSesion.setObject("IdCotizacion", smth.ToString());
        ...
        private CotizacionDetalleDTO configureCotDetItem(CotizacionDetalleDTO oItem)
        {
            ...
            lstProp.Add(new PropertyControl() { Tag = MultiFlujo.Tag.CotDetalle.Campo.UnidadMedida, IsVisible = true, IsEnabled = true, IdControl = "DI_pnlInfoGeneral_UnidadMedida" });
            lstProp.Add(new PropertyControl() { Tag = MultiFlujo.Tag.CotDetalle.Campo.Moneda, IsVisible = true, IsEnabled = true, IdControl = "DI_pnlCostos_Moneda" });
            lstProp.Add(new PropertyControl() { Tag = MultiFlujo.Tag.CotDetalle.Campo.ReqPlaca, IsVisible = true, IsEnabled = true, IdControl = "DI_pnlCostos_RequierePlaca" });
            ...
            if (oSolicitud.Tipo_Sol == ConstantesDTO.SolicitudVenta.TipoSolicitud.VentaEquipos)
            {
                ...
                if (oItem.TipoItem == ConstantesDTO.CotizacionVentaDetalle.TipoItem.Accesorio)
                {
                    lstProp.ForEach(pc =>
                    {
                        ...
                        if (pc.Tag == MultiFlujo.Tag.CotDetalle.Campo.UnidadMedida) { pc.IsVisible = false; pc.IsEnabled = false; };
                        if (pc.Tag == MultiFlujo.Tag.CotDetalle.Campo.Moneda) { pc.IsVisible = false; pc.IsEnabled = false; };
                        if (pc.Tag == MultiFlujo.Tag.CotDetalle.Campo.ReqPlaca) { pc.IsVisible = false; pc.IsEnabled = false; };
        ...
        private CotizacionDetalleDTO findSubCotDetRecord(string CodItemPadre, string CodItem)
        {
            ...
            {
                var cot = VariableSesion.getCadena("IdCotizacion");
                long idCotizacion = Int64.Parse(cot);
                var result = ventaBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = idCotizacion });
                ...
                if (result.Result != null)
                {
                    ...
                    {
                        var resArticulos = ventaBL.ObtenerArticulosxFiltro(new FiltroArticuloDTO() { CodsArticulo = list[i].CodItem });
                ...
                {
                    lstItems.Add(configureCotDetItem(x));
                });
            }
            else if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaMateriales)
            {
                var cot = VariableSesion.getCadena("IdCotizacion");
                long idCotizacion = Int64.Parse(cot);
                var result = ventaBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = idCotizacion });
        ...
        [HttpPost]
        public JsonResult CargarCotDetItemAnt(CotizacionDetalleDTO CotizacionDetalle, string opcGrillaItems)
        {
            try
            {
                var ventasBL = new VentasBL();
        ...
        public JsonResult GrabarDatosCotDetItem(CotizacionDetalleDTO CotizacionDetallePadre, CotizacionDetalleDTO CotizacionDetalle, string opcGrillaItems)
        {
            try
            {
                ...
                oCotDetItem.CotizacionDespacho = new CotDetDespachoDTO() { ObsCliente = oCotDesp.ObsCliente };
        ...
        public JsonResult EnviarCotizacion(CotizacionDTO oCotizacion)
        {
            try
            {
                ...
                if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaEquipos)
                {
                    lstItems = ventasBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = oCotizacion.IdCotizacion }).Result.ToList();
                }
                else if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaMateriales)
                {
                    lstItems = ventasBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = oCotizacion.IdCotizacion }).Result.ToList();
                ...
                if (
                    VariableSesion.getCadena("tipoSol") != ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaEquipos &&
                    VariableSesion.getCadena("tipoSol") != ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaMateriales
                    )
                {
                    if (lstItems != null)
                    {
        ...
        public JsonResult ListarCDCostosItems(CotDetCostoDTO oCosto)
        {
            ...
            if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaEquipos)
            {
                lstItems = ventasBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = oCosto.IdCotizacion }).Result.ToList();
            }
            else if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaMateriales)
            {
                lstItems = ventasBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = oCosto.IdCotizacion }).Result.ToList();
        ...
        public JsonResult CargarDatosCostoItem(CotDetCostoDTO cotdetCosto, string opcGrilla)
        {
            ...
            if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaEquipos)
            {
                lstCostos = ventasBL.ObtenerCotDetCostos(new CotDetCostoDTO() { Id = cotdetCosto.Id }).Result.ToList();
            }
            else if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaMateriales)
            {
                lstCostos = ventasBL.ObtenerCotDetCostos(new CotDetCostoDTO() { Id = cotdetCosto.Id }).Result.ToList();
            ...
            if (cdcItem.IdCotizacionDetalle != 0)
            {
                ...
                if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.DatosGenerales.TipoSolicitud.Valor1.VentaEquipos)
                {
                    var result = ventasBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = Int64.Parse(VariableSesion.getCadena("IdCotizacion")) });
                ...
                {
                    var result = ventasBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = cdcItem.IdCotizacion });
                ...
                else
                {
                    ...
                    if (envio_log.Result.Codigo > 0)
                    {
                        ...
                        if (idDespacho > 0)
                        {
                            ...
                            log.CodigoWorkflow = long.Parse(CodWorkFlow);
                            log.Usuario = User.ObtenerUsuario();
                            log.CodigoEstado = ConstantesDTO.EstadosProcesos.Despacho.PorFacturar;
                            log.UsuarioRegistro = User.ObtenerUsuario();
                            var result2 = procesoBL.InsertarWorkflowLog(log);
                        }
        ...
        public JsonResult EnviarGestionVentaConStock(DatosDespachoDTO datosDespachoDTO)
        {
            ...
            try
            {
                ...
                filtros.Usuario = User.ObtenerUsuario();
                filtros.Codigo = Convert.ToInt32(datosDespachoDTO.IdDespacho);
        ...
        [HttpPost]
        public JsonResult EnviarGestionServicioTecnico(long codigoDespacho)
        {
            var result = new RespuestaDTO();
            var ventasBL = new VentasBL();
            try
            {
                ...
                filtros.Usuario = User.ObtenerUsuario();
                filtros.Codigo = Convert.ToInt32(codigoDespacho);
                ...
                if (respuesta != "OK")
                {
                    Log.TraceInfo("Código de Despacho N° " + codigoDespacho.ToString() + ":" + respuesta);

                    result.Codigo = 0;
                    result.Mensaje = "No se pudo enviar el correo del Código de Despacho N° " + codigoDespacho.ToString();
                }
                else
                {
                    Log.TraceInfo("Envio exitoso para servicio tecnico del Código de Despacho N° " + codigoDespacho.ToString());


                    result.Codigo = 1;
                    result.Mensaje = "Se realizó correo al servicio tecnico del Código de Despacho N° " + codigoDespacho.ToString();
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (envio_log.Result.Codigo > 0)
                    {
                        ...
                        if (VariableSesion.getCadena("tipoSol") == ConstantesDTO.SolicitudVenta.TipoSolicitud.ServiciosyRepuestos)
                        {
                            if (datosDespachoDTO.IdDespacho > 0)
                            {
                                var procesoBL = new ProcesosBL();
                                ...
                                log.CodigoWorkflow = long.Parse(CodWorkFlow);
                                log.Usuario = User.ObtenerUsuario();
                                log.CodigoEstado = ConstantesDTO.EstadosProcesos.Despacho.Registrado;
                                log.UsuarioRegistro = User.ObtenerUsuario();
                                var result2 = procesoBL.InsertarWorkflowLog(log);
                            }
                        }
                        else
                        {
                            if (datosDespachoDTO.IdDespacho > 0)
                            {
                                ...
                                log.CodigoWorkflow = long.Parse(CodWorkFlow);
                                log.Usuario = User.ObtenerUsuario();
                                log.CodigoEstado = ConstantesDTO.EstadosProcesos.Despacho.Finalizado;
                                log.UsuarioRegistro = User.ObtenerUsuario();
                                var result2 = procesoBL.InsertarWorkflowLog(log);
                            }
                        }
                    ...
                    else
                    {
                        Log.TraceInfo("Solicitud N° " + datosDespachoDTO.CodigoSolicitud.ToString() + ":" + envio_log.Result.Mensaje);
                        result.Codigo = 0;
                        result.Mensaje = "No se pudo actualizar el despacho de la solicitud N° " + datosDespachoDTO.CodigoSolicitud.ToString();
                    }
        ...
        public JsonResult MantCosteoCotizacion(CosteoCotizacionDTO costo)
        {
            ...
            //Validamos sí ya se ha ingresado costos para accesorios

            var result = ventasBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = Int64.Parse(VariableSesion.getCadena("IdCotizacion")) });
            if (result.Result.Any(x => x.Id == costo.CodigoCotizacionDetalle && x.CotizacionDespacho.Id == 0 && x.TipoItem == "ACC")) //Confirmamos que para el detalle que consultamos, no cuente con costos registrados. 
            {//Caso de no tener costos procedemos a insertar
                costo.Tipo = "I";
            };

            var response = ventasBL.MantCosteoCotizacion(costo);

            result = ventasBL.ObtenerCotizacionVentaDetalle(new CotizacionDetalleDTO() { IdCotizacion = Int64.Parse(VariableSesion.getCadena("IdCotizacion")) });

            var total = result.Result.Count();
            var real = 0;
            foreach (var element in result.Result)
            {
                if (element.VentaUnitaria > 0)
                {
                    real += 1;
                };
            };
        ...
        [HttpPost]
        public JsonResult SolicitarDscto(CotizacionDTO cotizacion)
        {
            var rpta = new RespuestaDTO();
            ...
            {
                result.Result.Codigo = 0;
                result.Result.Mensaje = ex.Message.ToString();
            }
            return Json(result);
        }
        ...
        public ActionResult DetalleDespacho()
        {
            ...
            if (VariableSesion.getCadena("NumDespacho") != "0")
            {
                ...
                VariableSesion.setCadena("CodigoWorkFlowDesp", cod_workflow.ToString());
            ...
            //Botones:
            ViewBag.Btn_EnviarGuiaTotal = "none"; // OK
            ViewBag.Btn_GuiaPedidoTotal = "none"; // OK
            ViewBag.Btn_EnviarServicio = "none"; // OK
            ViewBag.Btn_GuiaManuscritaTotal = "none";  // OK
            ViewBag.Btn_Aprobar = "none"; // OK
            ...
            if (VariableSesion.getCadena("NumDespacho") != "0")
            {
                ...
                ViewBag.ColumnasServicios = ColumnasServicios;
        ...
        public JsonResult InsertDespacho(GrupoReqDespacho grupo)
        {
            try
            {
                ...
                var rptaMensaje = EnviarGestionServicioTecnico(result.Result.Codigo);
        ...
        public void LogicaBotones()
        {
                   ...
                   || NombreRol == ConstantesDTO.WorkflowRol.Venta.CoordAtc)
            {
                ...
                if (cod_estado == ConstantesDTO.EstadosProcesos.Despacho.Registrado)
                {
                                ...
                                tipoSol == ConstantesDTO.SolicitudVenta.TipoSolicitud.ServiciosyRepuestos)
                    {
                        if (validarDespacho.Result.EnvioServicio == 0)
                        {
                            ...
                            {
                                ViewBag.Btn_GuiaManuscritaTotal = "inline-block"; // se activa Guia Manuscrita
                                if (validarDespacho.Result.GenerarGuiaManuscrita > 0)
                                {
                                    ViewBag.Btn_EnviarServicio = "inline-block";
                                }
                                ...
                                //{
                                //    ViewBag.Btn_GuiaManuscritaTotal = "inline-block";
                                //    //if (tipoSol == C  onstantesDTO.SolicitudVenta.TipoSolicitud.Servicio && validarDespacho.Result.GenerarGuiaManuscrita > 0)
                                //    //{
                                //    //    ViewBag.Btn_EnviarServicio = "inline-block";
                                //    //}
                                //
                                ...
                                //{
                                //    ViewBag.Btn_GuiaManuscritaTotal = "inline-block";
                                //    if (tipoSol == ConstantesDTO.SolicitudVenta.TipoSolicitud.Servicio && validarDespacho.Result.GenerarGuiaManuscrita > 0)
                                //    {
                                //        ViewBag.Btn_EnviarServicio = "inline-block";
                                //    }
                                //}
                ...
                else if (cod_estado == ConstantesDTO.EstadosProcesos.Despacho.PorFacturar)
                {
