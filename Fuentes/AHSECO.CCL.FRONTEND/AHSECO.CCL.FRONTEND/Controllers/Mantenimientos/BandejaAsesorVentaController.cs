...
{
    public class BandejaAsesorVentaController : Controller
    {
        // GET: BandejaAsesorVenta
        [Permissions(Permissions = "BANDEJAASESORVENTA")]
        public ActionResult Index()
        {
            return View("Index");
        }
        [Permissions(Permissions = "BANDEJAASESORVENTA")]
        public ActionResult RegistrarAsesorVenta()
        {
            return View("RegistrarAsesorVenta");
        }

        public JsonResult ObtenerAsesorVenta(FiltroEmpleadosDTO filtroEmpleadosDTO)
        {
        ...
        public JsonResult InsertarAsesorVenta(FiltroEmpleadosDTO filtroEmpleadosDTO)
        {
            var empleadosBL = new EmpleadosBL();
            filtroEmpleadosDTO.UsuarioRegistra = User.ObtenerUsuario();
            filtroEmpleadosDTO.CodigoCargo = Convert.ToInt32(Core.ConstanteSesion.CodigoCargoAsesorVenta);
            var response = empleadosBL.MantenimientoEmpleado(filtroEmpleadosDTO);
            return Json(response);
        }


        public void ExportarAsesores(FiltroEmpleadosDTO filtroEmpleadosDTO)
        {
            var empleadosBL = new EmpleadosBL();
            var listaAsesores = empleadosBL.ListarEmpleados(filtroEmpleadosDTO).Result.ToList();

            var hssfworkbook = new HSSFWorkbook();
            ISheet sh = hssfworkbook.CreateSheet("Asesores");
            ...
            cell = row.CreateCell(cellnum++);
            cell.CellStyle = style;
            cell.SetCellValue("Apellido Paterno Asesor");

            cell = row.CreateCell(cellnum++);
            cell.CellStyle = style;
            cell.SetCellValue("Apellido Materno Asesor");

            cell = row.CreateCell(cellnum++);
            cell.CellStyle = style;
            cell.SetCellValue("Nombre Completo Asesor");
            ...
            var filename = "REPORTE_ASESOR_" + DateTime.Now.ToString("ddMMyyyyHHmmss") + ".xls";
