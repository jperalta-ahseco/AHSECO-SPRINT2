...
namespace NPOI.Extension
{
    ...
    public static class IEnumerableNpoiExtensions
    {
        ...
        public static IWorkbook ToWorkBook<T>(this IEnumerable<T> source, string sheetName = "sheet0", ICellStyle titleStyle = null, ICellStyle cellStyle = null, int indexFirstRow=0)
        {
            if (source == null)
            {
                throw new ArgumentNullException(nameof(source));
            ...
            return source.ToWorkbook(null, sheetName, titleStyle, cellStyle, indexFirstRow);

        }

        internal static IWorkbook ToWorkbook<T>(this IEnumerable<T> source, string excelFile, string sheetName, ICellStyle titleStyle=null, ICellStyle cellStyle=null, int indexFirstRow=0)
        {
            ...
            for (var j = 0; j < properties.Length; j++)
            {
                ...
                else
                {
					...
					{
                        cellConfigs[j] = new CellConfig();
                        cellConfigs[j].IsIgnored = true;
            ...
            var titleRow = sheet.CreateRow(indexFirstRow);
            
            var rowIndex = indexFirstRow+1;
            foreach (var item in source)
            {
                ...
                for (var i = 0; i < properties.Length; i++)
                {
                    ...
                    if (rowIndex == indexFirstRow+1)
                    {
                        ...
                        var copyStyle = workbook.CreateCellStyle();
                        //workbook.NumCellStyles;
                        if (copyStyle == null)
                            copyStyle = workbook.CreateCellStyle();

                        if (titleStyle != null)
                        {
                            //IFont f = copyStyle.GetFont(workbook);
                            //copyStyle.CloneStyleFrom(titleStyle);
                            //copyStyle.SetFont(f);
                            cloneStyle(titleStyle, copyStyle);

                        }
                        titleCell.CellStyle = copyStyle;
                    ...
                    if (value!=null && cellStyles.TryGetValue(i, out var newCellStyle))
                    {
                    ...
                    {
                        cellStyles.TryGetValue(i, out newCellStyle);
                        cell.CellStyle = newCellStyle;
                        var fv = value as IFormattable;
                        cell.SetCellValue(fv.ToString(config.Formatter, CultureInfo.CurrentCulture));
                    }
        ...
        private static void cloneStyle(ICellStyle source, ICellStyle dest) {
            dest.Alignment = source.Alignment;
            dest.VerticalAlignment = source.VerticalAlignment;
            dest.WrapText = source.WrapText;
            dest.BorderLeft = source.BorderLeft;
            dest.BorderTop = source.BorderTop;
            dest.BorderRight = source.BorderRight;
            dest.BorderBottom = source.BorderBottom;
        }
